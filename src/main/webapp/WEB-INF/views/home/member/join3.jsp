...
<html lang="ko" xmlns="http://www.w3.org/1999/xhtml"><head>
   ...
   <link type="text/css" rel="stylesheet" href="../../../../resources/css/reservation.css?ver=1">
	...
	<link type="text/css" rel="stylesheet" href="../../../../resources/css/join3.css">
    ...
    <script type="text/javascript">
      	//본인인증 팝업
		/*
      	function fn_IdentityVerification() {
			window.open("","ivPop","width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no");
			$("#frmIV").attr("action","https://nice.checkplus.co.kr/CheckPlusSafeModel/checkplus.cb");
			$("#frmIV").attr("target","ivPop");
			$("#frmIV").submit();
		}
      	*/
      	/*
    	function fn_IdentityVerification() {
			window.open("","ivPop","width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no");
			$("#frmIV").attr("action","http://localhost:8080/home/member/bonin.do");
			$("#frmIV").attr("target","ivPop");
			$("#frmIV").submit();
		}
		//본인인증 콜백 - 성공
		function fn_IdentityVerificationSuccess(json) {
			if (json != null && json != undefined) {
				$("#frmIV > #userNm").val(json.userNm);
				$("#frmIV > #authType").val(json.authType);
				$("#frmIV > #userDi").val(json.di);
				$("#frmIV").attr("action","/home/member/join3Proc.do");
				$("#frmIV").attr("target","_self");
				$("#frmIV").submit();
			} else {
				alert("본인인증에 실패하였습니다.");
				return false;
		...
		*/
		//본인인증 콜백 - 실패
		function fn_IdentityVerificationFail(json) {}
		
		//아이핀 인증 팝업
		function fn_IpinVerification() {
			window.open("","ipinPop","width=450, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no");
			$("#frmIpin").attr("action","https://cert.vno.co.kr/ipin.cb");
			$("#frmIpin").attr("target","ipinPop");
			$("#frmIpin").submit();
		}
		//아이핀 인증 콜백 - 성공
		function fn_IpinVerificationSuccess(json) {
			//alert(json.toString());return false;
			if (json != null && json != undefined) {
				$("#frmIpin > #userNm").val(json.userNm);
				$("#frmIpin > #authType").val(json.authType);
				$("#frmIpin > #userDi").val(json.di);
				$("#frmIpin").attr("action","/home/member/join3Proc.do");
				$("#frmIpin").attr("target","_self");
				$("#frmIpin").submit();
			} else {
				alert("본인인증에 실패하였습니다.");
				return false;
		...
		//본인인증 콜백 - 실패
		function fn_IpinVerificationFail(json) {}
    </script>
	<!-- 이메일 인증 추가 코드 -->
...
<a href="javascript:fn_goMenu('/home/member/findId.do');" class="title_next">Next</a>
    ...
    <!-- 
    <form id="frmIV" name="frmIV" method="POST">
        ...
        <input type="hidden" id="EncodeData" name="EncodeData" value="">
        <input type="hidden" id="userNm" name="userNm">
        <input type="hidden" id="authType" name="authType">
        <input type="hidden" id="userDi" name="userDi">
    ...
    <!-- 
    <form id="frmIpin" name="frmIpin" method="POST">
        <input type="hidden" id="accessTermsYn" name="accessTermsYn" value="Y">
        <input type="hidden" id="accessPerYn" name="accessPerYn" value="Y">
        <input type="hidden" id="loginType" name="loginType" value="S">
        <input type="hidden" id="userTp" name="userTp" value="Y">
        <input type="hidden" id="m" name="m" value="pubmain">
        <input type="hidden" id="enc_data" name="enc_data" value="">
        <input type="hidden" id="userNm" name="userNm">
        <input type="hidden" id="authType" name="authType">
        <input type="hidden" id="userDi" name="userDi">
    <input type="hidden" id="headHspCd" name="headHspCd" value="G">
	...
	<section id="contents" class="contents_wrap">
		<div class="inner">
			<div class="certification_wrap">
				...
				<div class="con_sec">
					<h3 class="sub_tit">회원가입안내 </h3>
					<div class="notice_gray_box">
						<p class="notice_tit">반드시 본인의 실명으로 가입해 주시기 바랍니다.</p>
						<p class="con_txt">
							타인의 개인정보를 도용하여 온라인 회원가입을 하는 경우, 주민등록법에 따라 3년 이하의 징역 또는 1천만원 이하의 벌금이 부과될 수 있습니다.<br>
							- 온라인 회원이 되시면 다양한 서비스를 이용하실 수 있습니다.<br>
							- 중앙대학교광명병원은 고객님께 만족스러운 서비스를 제공하기 위하여 항상 노력하겠습니다.
						</p>
					</div>
				</div>
				<div class="ssgap"></div>
				<div class="choose_box_wrap">
					<dl class="choose_box">
						<dt class="tit sty3">
							<strong class="txt">본인인증</strong>
                     <dd id="mine">
                     <p id="n">이름</p> <input type="text" id="userNm" name="userNm"><br>
                     </dd>
                     <dd id="mine2">
                     <p id="ju">주민등록번호</p><br>
                     <input type="text" id="authType" name="authType"><br>
                      <p id="aa">-</p> <input type="password" id="userDi" name="userDi">
                     </dd>
                  </dt>
                  <dd class="dec">
                     <p class="con_txt">
                     인터넷 상에서의 실명확인 시 실제 본인 여부의 확인을 위한<span class="m_block"></span>
                     본인인증 서비스 실제 실명확인을 받는 사람의 명의 확인이 가능해<span class="m_block"></span>
                     주민등록번호 도용을 원천 방지할 수 있습니다.
                     </p>
							<!--   
							<a href="javascript:void(0);" onclick="javascript:fn_IdentityVerification();" class="btn_ty01_sty01"><span>본인인증하기</span></a>
							 -->
							 
							 <input type="submit" formaction="join4.do" value="본인인증하기" class="btn_ty01_sty01">
						</dd>
					</dl>
					<dl class="choose_box">
						<dt class="tit sty4">
							<strong class="txt">아이핀(I-PIN)</strong>
						</dt>
						<dd class="dec">
							<p class="con_txt">
							<br>
							...
							<a href="javascript:void(0);" onclick="javascript:fn_IpinVerification();" class="btn_ty01_sty01"><span>본인인증하기</span></a>
						</dd>
					</dl>
				</div>
			</div>
