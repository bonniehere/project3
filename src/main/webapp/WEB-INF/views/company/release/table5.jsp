    ...
    <c:otherwise>
        ...
        <div id="tabViewer" class="tabViewer">
            <div id="tabViewer-tabMenu" class="tabViewer-tabMenu">
                ...
                <input type="button" value="소독/의약제품" class="tabViewer-tab active" id="menu_tab" onclick="location.href='/table5';">
            </div>
            ...
            <div class="paging">
                ...
                <c:if test="${pageMaker.prev}">
                    <a href="/table5?pageNum=${pageMaker.startPage-1}&amount=${pageMaker.cri.amount}">이전</a>
                </c:if>

                <!-- begin(1)이 end(10)될 동안 반복(1일 10일 될 동안 반복) -->
                <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="num">
                    <a href="/table5?&pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a>
                </c:forEach>

                <!-- next(다음)이 true이면 다음버튼 활성화 -->
                <c:if test="${pageMaker.next}">
                    <a href="/table5?&pageNum=${pageMaker.endPage+1}&amount=${pageMaker.cri.amount}">다음</a>
                </c:if>
            </div>
        </div>
    </div>
